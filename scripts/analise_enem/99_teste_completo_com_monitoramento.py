#!/usr/bin/env python3
"""
🧪 TESTE COMPLETO COM MONITORAMENTO EM TEMPO REAL

Testa com 45 questões e monitora progresso em tempo real.

Uso:
    python 99_teste_completo_com_monitoramento.py --area natural-sciences
"""
...
def imprimir_status(questao_atual: int, total: int, corretos: int, tempo_decorrido: float, 
                   respostas_preditas: Counter, respostas_corretas: Counter):
    """Imprime status atual"""
    acuracia_atual = (corretos / questao_atual * 100) if questao_atual > 0 else 0
    tempo_medio = tempo_decorrido / questao_atual if questao_atual > 0 else 0
    tempo_restante = tempo_medio * (total - questao_atual)
    
    print(f"\n{'='*70}")
    print(f"📊 PROGRESSO: {questao_atual}/{total} questões ({questao_atual/total*100:.1f}%)")
    print(f"{'='*70}")
    print(f"✅ Acertos: {corretos}/{questao_atual}")
    print(f"🎯 Acurácia atual: {acuracia_atual:.2f}%")
    print(f"⏱️  Tempo decorrido: {tempo_decorrido/60:.1f} min")
    print(f"⏱️  Tempo médio por questão: {tempo_medio:.1f}s")
    print(f"⏱️  Tempo estimado restante: {tempo_restante/60:.1f} min")
    print()
    print("📊 DISTRIBUIÇÃO ATUAL:")
    print("   Preditas: ", end="")
    for letra in ['A', 'B', 'C', 'D', 'E']:
        count = respostas_preditas.get(letra, 0)
        pct = (count / questao_atual * 100) if questao_atual > 0 else 0
        print(f"{letra}:{count}({pct:.0f}%) ", end="")
    print()
    print("   Corretas:  ", end="")
    for letra in ['A', 'B', 'C', 'D', 'E']:
...
def main():
    parser = argparse.ArgumentParser(description='Teste completo com monitoramento')
    ...
    parser.add_argument('--passagens', type=int, default=3,
                       help='Número de passagens')
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("🧪 TESTE COMPLETO COM MONITORAMENTO")
    print("=" * 70)
    print()
    ...
    # Carregar questões
    print(f"\n📥 Carregando {args.limit} questões de {args.area}...")
    ...
    for i, questao in enumerate(questoes, 1):
        num = questao.get('number', 0)
        questao_start = time.time()
        
        print(f"[{i}/{len(questoes)}] Q{num}: ", end='', flush=True)
    ...
    print(f"⏱️  Tempo total: {elapsed/60:.1f} min ({elapsed/stats['total']:.1f}s por questão)")
    print()
    ...
    if count_e_predita > count_e_correta * 1.5:
        print(f"⚠️  VIÉS DETECTADO: E escolhida {count_e_predita} vezes ({pct_e_predita:.1f}%)")
    ...
    # Salvar resultados
    output_file = Path(__file__).parent.parent.parent / "results" / f"teste_completo_{args.area}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
